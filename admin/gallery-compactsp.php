<?php
	...
	function get_image()
	{
		$img_list = glob("../gallerysp/*.{jpg,JPG,jpeg,JPEG,gif,GIF,png,PNG,bmp,BMP}", GLOB_BRACE);
	...
	function gen_paging($tpage, $cpage)
	{
		...
		if($tpage > 1)
		{
			...
			{
				$paging .= '<a class="btn btn-default" href="?m=gallerysp&page=1" role="button">
					<i class="fa fa-angle-double-left" style="font-size:20px"></i></a>';
			}
			if($cpage > 1)	// add previous button
			{
				$paging .= '<a class="btn btn-default" href="?m=gallerysp&page='.($cpage-1).'" role="button">
			...
			for($i = $start; $i <= $end; $i++)
			{
				...
				$paging .= '<a class="btn '.$class.'" href="?m=gallerysp&page='.$i.'" role="button">'.$i.'</a>';
			}
			...
			{
				$paging .= '<a class="btn btn-default" href="?m=gallerysp&page='.$tpage.'" role="button">
    ...
    <body>
    	<div class="container">
            ...
            <?php
				...
				{
					$image_name = str_replace('../gallerysp/','gallerysp/',$filename);
					$name = str_replace('../gallerysp/','',$filename);
